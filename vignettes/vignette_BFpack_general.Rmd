...
`BF(x, hypothesis, prior.hyp.explo = NULL, prior.hyp.conf = NULL, complement = TRUE, ...)`
...
In the above test the complement hypothesis is excluded automatically as the formulated hypothesis under the `hypothesis` argument cover the complete parameter space. Furthermore, when testing hypotheses via the `hypothesis` argument, the output also presents an `Evidence matrix` containing the Bayes factors between the hypotheses formulated in the `hypothesis` argument.
...
The argument `prior.hyp.conf` can be used to specify different prior probabilities
for the hypotheses formulated under the `hypothesis` argument. For example, when the left one-tailed hypothesis is not possible
...
one-tailed hypothesis are equally likely, the argument `prior.hyp.conf` should be a vector
specifying the prior probabilities of the respective hypotheses
``` r
BF(ttest1, hypothesis = "mu = 5; mu < 5; mu > 5", prior.hyp.conf = c(.5,0,.5))
```
...
executed of whether an effect is zero, negative, or positive.
...
For more information about the methodology, we refer the interested reader to [Boeing-Messing et al. (2017)](https://www.researchgate.net/publication/317418250_Bayesian_evaluation_of_constrained_hypotheses_on_variances_of_multiple_independent_groups)
...
For more information about the methodology, we refer the interested reader to [Van Aert and Mulder (2021)](https://link.springer.com/article/10.3758/s13423-021-01918-9) and [Mulder and van Aert (2025)](https://doi.org/10.1017/rsm.2025.10060).


### Exponential random graph models

`BFpack` can be used for Bayes factor testing of the coefficients in an exponential random graph model (ERGM). As input the `BF()` function requires a fitted ERGM either from the `ergm` package or the `Bergm` package. Bayes factors are then computed using a so-called unit-information prior. For an ERGM, this prior contains the information of single dyad. Moreover, the prior is centered at 0. For Bayes factor testing, this prior is preferred to an arbitrarily vague prior (Mulder, Friel, \& Leifeld, 2024). For this illustration, we use the `florentine` data from the `ergm` package and we fit the following ERGM:
...
A Bayes factor is computed for a traditional two-sided test of whether the absolute difference in wealth between two families affects tie formation.

``` r
set.seed(123)
BFergm <- BF(ergm_fit,
  hypothesis = "absdiff.wealth = 0",
  main.iters = 500)
print(BFergm)
```

The null hypothesis is not supported give the Bayes factor of 9.36 in favor of the two-sided alternative, and given the posterior probability of 0.097 for the null. 

Finally note that one could retrieve the employed unit-information prior from the `BF` object:

``` r
BFergm$prior.parameters
```

For more information about the methodology, we refer the interested reader to [Mulder, Friel, and Leifeld (2024)](https://doi.org/10.1016/j.socnet.2023.11.004).

### Hypothesis testing using normal approximations

For several model classes, Bayes factors are computed using Gaussian (large-sample) approximations of the likelihood and posterior, in the spirit of the classical Wald test. This applies by default to fitted objects of class `glm`, `coxph`, `survreg`, `polr`, and `zeroinfl`, and can also be used directly by supplying a named vector of estimates together with their error covariance matrix and sample size (illustrated below).
...
can be formulated can be extracted using the `get_estimates` function.
